...
void CLOCK_VT_Start(void) 
{
    ...
    CLOCK_VT_CLKEN |= CLOCK_VT_CLKEN_MASK;
	CLOCK_VT_CLKSTBY |= CLOCK_VT_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: CLOCK_VT_Stop
********************************************************************************
*
* Summary:
...
void CLOCK_VT_Stop(void) 
{
    ...
    CLOCK_VT_CLKEN &= (uint8)(~CLOCK_VT_CLKEN_MASK);
	CLOCK_VT_CLKSTBY &= (uint8)(~CLOCK_VT_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: CLOCK_VT_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((CLOCK_VT_CLKEN & CLOCK_VT_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = CLOCK_VT_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = CLOCK_VT_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* CLOCK_VT__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(CLOCK_VT_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CLOCK_VT_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: CLOCK_VT_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: CLOCK_VT_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = CLOCK_VT_GetSourceRegister();
    uint16 oldDivider = CLOCK_VT_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CLOCK_VT_CLKEN & CLOCK_VT_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CLOCK_VT_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: CLOCK_VT_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(CLOCK_VT_DIV_PTR);
}


/*******************************************************************************
* Function Name: CLOCK_VT_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    CLOCK_VT_MOD_SRC |= modeBitMask & (uint8)CLOCK_VT_MODE_MASK;
}


/*******************************************************************************
* Function Name: CLOCK_VT_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    CLOCK_VT_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CLOCK_VT_MODE_MASK));
}


/*******************************************************************************
* Function Name: CLOCK_VT_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return CLOCK_VT_MOD_SRC & (uint8)(CLOCK_VT_MODE_MASK);
}


/*******************************************************************************
* Function Name: CLOCK_VT_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = CLOCK_VT_GetDividerRegister();
    uint8 oldSrc = CLOCK_VT_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CLOCK_VT_MOD_SRC |= CYCLK_SSS;
        CLOCK_VT_MOD_SRC =
            (CLOCK_VT_MOD_SRC & (uint8)(~CLOCK_VT_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CLOCK_VT_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return CLOCK_VT_MOD_SRC & CLOCK_VT_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: CLOCK_VT_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    CLOCK_VT_PHASE = clkPhase & CLOCK_VT_PHASE_MASK;
}


/*******************************************************************************
* Function Name: CLOCK_VT_GetPhase
********************************************************************************
*
* Summary:
...
{
    return CLOCK_VT_PHASE & CLOCK_VT_PHASE_MASK;
}
