...
/*******************************************************************************
* Function Name: CLOCK_PWM_TOAST_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CLOCK_PWM_TOAST_Start(void) 
{
    /* Set the bit to enable the clock. */
    CLOCK_PWM_TOAST_CLKEN |= CLOCK_PWM_TOAST_CLKEN_MASK;
	CLOCK_PWM_TOAST_CLKSTBY |= CLOCK_PWM_TOAST_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: CLOCK_PWM_TOAST_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CLOCK_PWM_TOAST_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    CLOCK_PWM_TOAST_CLKEN &= (uint8)(~CLOCK_PWM_TOAST_CLKEN_MASK);
	CLOCK_PWM_TOAST_CLKSTBY &= (uint8)(~CLOCK_PWM_TOAST_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: CLOCK_PWM_TOAST_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CLOCK_PWM_TOAST_StopBlock(void) 
{
    if ((CLOCK_PWM_TOAST_CLKEN & CLOCK_PWM_TOAST_CLKEN_MASK) != 0u)
    {
...
#if defined(CLOCK_PWM_TOAST__CFG3)
        CLK_DIST_AMASK = CLOCK_PWM_TOAST_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = CLOCK_PWM_TOAST_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* CLOCK_PWM_TOAST__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(CLOCK_PWM_TOAST_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CLOCK_PWM_TOAST_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: CLOCK_PWM_TOAST_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CLOCK_PWM_TOAST_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        CLOCK_PWM_TOAST_CLKSTBY &= (uint8)(~CLOCK_PWM_TOAST_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: CLOCK_PWM_TOAST_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CLOCK_PWM_TOAST_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = CLOCK_PWM_TOAST_GetSourceRegister();
    uint16 oldDivider = CLOCK_PWM_TOAST_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CLOCK_PWM_TOAST_CLKEN & CLOCK_PWM_TOAST_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(CLOCK_PWM_TOAST_DIV_PTR, clkDivider);
                CLOCK_PWM_TOAST_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                CLOCK_PWM_TOAST_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(CLOCK_PWM_TOAST_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CLOCK_PWM_TOAST_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: CLOCK_PWM_TOAST_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 CLOCK_PWM_TOAST_GetDividerRegister(void) 
{
    return CY_GET_REG16(CLOCK_PWM_TOAST_DIV_PTR);
}


/*******************************************************************************
* Function Name: CLOCK_PWM_TOAST_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CLOCK_PWM_TOAST_SetModeRegister(uint8 modeBitMask) 
{
    CLOCK_PWM_TOAST_MOD_SRC |= modeBitMask & (uint8)CLOCK_PWM_TOAST_MODE_MASK;
}


/*******************************************************************************
* Function Name: CLOCK_PWM_TOAST_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CLOCK_PWM_TOAST_ClearModeRegister(uint8 modeBitMask) 
{
    CLOCK_PWM_TOAST_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CLOCK_PWM_TOAST_MODE_MASK));
}


/*******************************************************************************
* Function Name: CLOCK_PWM_TOAST_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 CLOCK_PWM_TOAST_GetModeRegister(void) 
{
    return CLOCK_PWM_TOAST_MOD_SRC & (uint8)(CLOCK_PWM_TOAST_MODE_MASK);
}


/*******************************************************************************
* Function Name: CLOCK_PWM_TOAST_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CLOCK_PWM_TOAST_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = CLOCK_PWM_TOAST_GetDividerRegister();
    uint8 oldSrc = CLOCK_PWM_TOAST_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CLOCK_PWM_TOAST_MOD_SRC |= CYCLK_SSS;
        CLOCK_PWM_TOAST_MOD_SRC =
            (CLOCK_PWM_TOAST_MOD_SRC & (uint8)(~CLOCK_PWM_TOAST_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CLOCK_PWM_TOAST_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 CLOCK_PWM_TOAST_GetSourceRegister(void) 
{
    return CLOCK_PWM_TOAST_MOD_SRC & CLOCK_PWM_TOAST_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: CLOCK_PWM_TOAST_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CLOCK_PWM_TOAST_SetPhaseRegister(uint8 clkPhase) 
{
    CLOCK_PWM_TOAST_PHASE = clkPhase & CLOCK_PWM_TOAST_PHASE_MASK;
}


/*******************************************************************************
* Function Name: CLOCK_PWM_TOAST_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 CLOCK_PWM_TOAST_GetPhaseRegister(void) 
{
    return CLOCK_PWM_TOAST_PHASE & CLOCK_PWM_TOAST_PHASE_MASK;
}
